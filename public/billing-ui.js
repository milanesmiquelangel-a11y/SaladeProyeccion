(() => {
  ...
  section.innerHTML = `<div class="section-heading"><div><p class="eyebrow">CUENTA Y MONETIZACIÓN</p><h2>Planes y créditos</h2><p class="muted">Suscríbete con PayPal y recibe créditos mensuales para producir más vídeos.</p></div><div class="billing-balance"><span>Saldo</span><strong id="billingCredits">0 créditos</strong></div></div>
    ...
    <div class="cards-grid pricing-grid">
      ...
      <article class="template-card pricing-card featured-plan"><span>🎬</span><h3>Creador</h3><p>Para producir contenido con frecuencia.</p><strong>4,99 € / mes</strong><small>30 créditos por ciclo</small><button class="primary plan-btn buy-plan" data-plan="creator" disabled>Comprobando…</button></article>
      <article class="template-card pricing-card"><span>🚀</span><h3>Pro</h3><p>Para producción continua y mayor volumen.</p><strong>14,99 € / mes</strong><small>100 créditos por ciclo</small><button class="secondary plan-btn buy-plan" data-plan="pro" disabled>Comprobando…</button></article>
    </div>
  ...
  section.querySelectorAll('.buy-plan').forEach((button) => button.addEventListener('click', async () => {
    ...
    try { const response = await fetch('/api/billing/checkout', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ plan }) }); const data = await response.json().catch(() => ({})); if (!response.ok) throw new Error(data.error || 'No se pudo iniciar la suscripción.'); if (!data.checkoutUrl) throw new Error('PayPal no devolvió la dirección de aprobación.'); window.location.href = data.checkoutUrl; }
