(() => {
  ...
  section.innerHTML = `<div class="section-heading"><div><p class="eyebrow">CUENTA Y MONETIZACIÓN</p><h2>Planes y créditos</h2><p class="muted">Compra créditos cuando necesites producir más vídeos. Los pagos se procesan mediante Stripe.</p></div><div class="billing-balance"><span>Saldo</span><strong id="billingCredits">0 créditos</strong></div></div>
    <div id="stripeStatus" class="panel billing-notice">Comprobando disponibilidad de pagos…</div>
    ...
    <div class="cards-grid pricing-grid">
      ...
      <article class="template-card pricing-card featured-plan"><span>🎬</span><h3>Creador</h3><p>Para producir contenido con frecuencia.</p><strong>4,99 €</strong><small>30 créditos</small><button class="primary plan-btn buy-plan" data-plan="creator" disabled>Comprobando…</button></article>
      <article class="template-card pricing-card"><span>🚀</span><h3>Pro</h3><p>Para producción continua y mayor volumen.</p><strong>14,99 €</strong><small>100 créditos</small><button class="secondary plan-btn buy-plan" data-plan="pro" disabled>Comprobando…</button></article>
    </div>
  ...
  const stripeStatus = section.querySelector('#stripeStatus');
  ...
  const renderTransactions = (items = []) => {
    ...
    history.innerHTML = items.map((item) => { const date = item.createdAt ? new Date(item.createdAt).toLocaleString() : ''; const label = item.route === '/api/video/sequence' ? 'Vídeo largo' : 'Generación de vídeo'; return `<div class="billing-transaction"><span><strong>${label}</strong><small>${date}</small></span><strong>−${item.cost} crédito${item.cost === 1 ? '' : 's'}</strong></div>`; }).join('');
  };
  const loadBalance = async () => {
    try { const response = await fetch('/api/billing/balance'); const data = await response.json(); if (!response.ok) throw new Error(data.error || 'No se pudo actualizar el saldo.'); const current = read(); localStorage.setItem(STORAGE_KEY, JSON.stringify({ ...current, credits: Number(data.credits || 0), plan: data.plan || 'Gratis', totalConsumed: Number(data.totalConsumed || 0), nextRechargeAt: data.nextRechargeAt || null, freeRechargeCredits: Number(data.freeRechargeCredits || 3) })); render(); } catch {}
  };
  const loadTransactions = async () => { try { const response = await fetch('/api/billing/transactions'); const data = await response.json(); if (!response.ok) throw new Error(data.error || 'No se pudo cargar la actividad.'); renderTransactions(data.transactions); } catch (error) { history.textContent = error.message || 'No se pudo cargar la actividad.'; } };
  const loadStripeStatus = async () => {
    const buttons = section.querySelectorAll('.buy-plan');
    try {
      const response = await fetch('/api/billing/status'); const data = await response.json();
      if (!response.ok) throw new Error(data.error || 'No se pudo consultar Stripe.');
      if (data.configured) {
        stripeStatus.textContent = data.testMode ? '🧪 Stripe está conectado en modo de prueba. Los pagos todavía no son reales.' : '✅ Pagos con Stripe disponibles.';
        buttons.forEach((button) => { button.disabled = false; button.textContent = 'Comprar créditos'; });
      } else {
        stripeStatus.textContent = '⏳ Pagos todavía no configurados. Faltan las credenciales y/o los Price ID de Stripe en Render.';
        buttons.forEach((button) => { button.disabled = true; button.textContent = 'Configurar Stripe'; });
      }
    } catch (error) { stripeStatus.textContent = error.message || 'No se pudo comprobar el estado de Stripe.'; }
  };
  const refreshAll = async () => { await Promise.all([loadBalance(), loadTransactions(), loadStripeStatus()]); };
  ...
  section.querySelectorAll('.buy-plan').forEach((button) => button.addEventListener('click', async () => {
    const plan = button.dataset.plan; button.disabled = true; button.textContent = 'Preparando pago…'; showPurchaseMessage('Preparando el pago seguro…');
    try { const response = await fetch('/api/billing/checkout', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ plan }) }); const data = await response.json().catch(() => ({})); if (!response.ok) throw new Error(data.error || 'No se pudo iniciar el pago.'); if (!data.checkoutUrl) throw new Error('Stripe no devolvió la dirección de pago.'); window.location.href = data.checkoutUrl; }
    catch (error) { showPurchaseMessage(error.message || 'No se pudo iniciar el pago.', true); button.disabled = false; button.textContent = 'Comprar créditos'; }
  }));
