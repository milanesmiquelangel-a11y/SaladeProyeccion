(() => {
  ...
  async function waitForAuth() {
    if (!window.salaAuthReady) return null;
    try { const user = await window.salaAuthReady; currentUserId = user?.id || null; return user; } catch { return null; }
  }
  ...
  async function refreshBalance() {
    ...
    if (!user) return;
    try {
      const response = await nativeFetch('/api/billing/balance', { credentials:'same-origin' });
